...
ChangeConf(i) ==
    ...
    /\ IF pendingConfChangeIndex[i] <= applied[i] THEN
            \E newVoters \in SUBSET Server, newLearners \in SUBSET Server, enterJoint \in {TRUE, FALSE}:
                ...
                \* Safety constraint: new config must overlap with old config for joint consensus
                \* Reference: doc.go "old and new configurations are guaranteed to overlap"
                /\ (enterJoint = TRUE) => (newVoters \cap GetConfig(i) /= {})
