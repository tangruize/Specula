...
ProposeLeaveJoint(i) ==
    ...
    /\ IsJointConfig(i)
    /\ config[i].autoLeave = TRUE
    \* BUG bd3c759: Removed check for pendingConfChangeIndex = 0
    \* Original: /\ pendingConfChangeIndex[i] = 0  \* Previous config change has been applied
    \* This allows multiple leave-joint entries to be proposed
    ...
    \* BUG bd3c759: Don't update pendingConfChangeIndex
    \* Original: /\ pendingConfChangeIndex' = [pendingConfChangeIndex EXCEPT ![i] = LastIndex(log'[i])]
    /\ UNCHANGED <<pendingConfChangeIndex>>
    /\ UNCHANGED <<messageVars, serverVars, candidateVars, matchIndex, commitIndex, applied, configVars, durableState, progressVars, partitions>>
