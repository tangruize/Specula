...
HandleAppendEntriesResponse(i, j, m) ==
    ...
    /\ \/ /\ m.msuccess \* successful
          ...
          /\ LET maybeUpdated == m.mmatchIndex > matchIndex[i][j]
                 ...
                 \* BUG 124: Progress State Transition Bug
                 \* Issue: https://github.com/etcd-io/raft/issues/124
                 \* The buggy condition only checks exact equality with pendingSnapshot,
                 \* not >=. This causes follower to get stuck in StateSnapshot if the
                 \* match index goes past the pending snapshot index.
                 \*
                 \* Original (fixed): newMatchIndex + 1 >= pendingSnapshot[i][j]
                 \* Buggy: newMatchIndex = pendingSnapshot[i][j] (exact match required)
                 canResumeFromSnapshot == \/ newMatchIndex + 1 >= log[i].offset
                                          \/ newMatchIndex = pendingSnapshot[i][j]  \* BUG: uses = instead of >=
...
\* ============================================================================
\* BUG 124: Progress State Transition Detection Invariant
\* Issue: https://github.com/etcd-io/raft/issues/124
\* When in StateSnapshot, if matchIndex >= pendingSnapshot, the follower should
\* transition out of StateSnapshot. The bug uses exact equality (=) instead of >=,
\* causing the follower to get stuck in StateSnapshot when matchIndex exceeds
\* pendingSnapshot.
\* ============================================================================

\* This invariant checks that if matchIndex > pendingSnapshot while in StateSnapshot,
\* something is wrong - with the bug, this can happen because the state transition
\* condition uses = instead of >=, causing the follower to remain stuck.
\* Violation: Follower stuck in StateSnapshot even though matchIndex > pendingSnapshot
SnapshotTransitionCorrectInv ==
    \A i, j \in Server :
        (state[i] = Leader /\ progressState[i][j] = StateSnapshot) =>
            \* matchIndex should NOT exceed pendingSnapshot while in StateSnapshot
            \* If it does, it means the state transition didn't happen (bug!)
            matchIndex[i][j] <= pendingSnapshot[i][j]

\* Alternative: Check that follower is not stuck when it should transition
SnapshotStateNotStuckInv ==
    \A i, j \in Server :
        (state[i] = Leader /\ progressState[i][j] = StateSnapshot /\
         matchIndex[i][j] >= pendingSnapshot[i][j] /\
         matchIndex[i][j] + 1 >= log[i].offset) =>
            \* Should have transitioned to Replicate, but with the bug, stays in Snapshot
            FALSE  \* This state should not be reachable with correct transition
