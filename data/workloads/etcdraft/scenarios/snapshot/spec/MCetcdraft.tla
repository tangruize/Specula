...
MCDynamicConfigActions ==
    ...
    \/ /\ \E i \in Server : etcd!ProposeLeaveJoint(i)
       /\ UNCHANGED faultVars
    \* NOTE: ApplySnapshotConfChange action REMOVED
    \* Config is already applied atomically in HandleSnapshotRequest (Case 3: Actual Restore)
    \* Reference: raft.go:1919-1934 - restore() calls r.raftLog.restore(s) then switchToConfig()
    \* Having a separate action would cause config regression bugs:
    \* - If config was updated after receiving snapshot (via normal ApplyConfigUpdate)
    \* - The separate action could reset config back to snapshot's old config
